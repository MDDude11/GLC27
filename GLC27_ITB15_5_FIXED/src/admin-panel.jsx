...
export default function AdminPanel() {
  ...
  const create = async (e) => {
    ...
    try {
      const existingIds = new Set(Object.keys(matches || {}));
      const makeToken = () => {
        const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        const digits = "0123456789";
        const pick = (source) => source[Math.floor(Math.random() * source.length)];
        const chars = [pick(alphabet), pick(alphabet), pick(alphabet), pick(digits), pick(digits), pick(digits), pick(digits)];
      ...
      let id = "";
      do {
        const head = Array.from({ length: 5 }, () => "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[Math.floor(Math.random() * 36)]).join("");
        id = `${INTERNAL_MATCH_PREFIX}${head}-${makeToken()}`;
