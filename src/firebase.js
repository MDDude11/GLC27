...
async function loadFirebase() {
  if (!firebasePromise) {
    firebasePromise = Promise.all([
      ...
      return { database, ref: databaseModule.ref, onValue: databaseModule.onValue, get: databaseModule.get, set: databaseModule.set };
    });
  }
...
export async function subscribeFirebaseMatch(matchId, onMatch, onError) {
  try {
    ...
    return onValue(
      ...
      (snapshot) => onMatch(snapshot.exists() ? snapshot.val() : null),
      (error) => {
        console.warn(`Firebase match subscription failed for ${matchId}.`, error);
        onError?.(error);
      }
    );
  } catch (error) {
    console.warn(`Firebase match subscription could not start for ${matchId}.`, error);
    onError?.(error);
    return () => {};
  }
}
...
export async function writeFirebaseInternalMatch(matchId, match) {
  const { database, ref, set } = await loadFirebase();
