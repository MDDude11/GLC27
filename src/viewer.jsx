...
export default function ViewerPage({ matchId }) {
  ...
  const viewerTeams = fixture.teams || TEAMS;
  const headline = state.status === "completed"
    ...
    : state.status === "innings2"
      ? "Innings break"
      : `${viewerTeams?.[fixture.t1]?.name || fixture.t1 || "Team A"} vs ${viewerTeams?.[fixture.t2]?.name || fixture.t2 || "Team B"}`;
  ...
  return <SiteFrame active="matches"><main className="section-wrap page-section viewer-page">
    ...
    <section className="match-view-hero comic-panel dark-panel"><div className="viewer-team"><TeamBadge code={fixture.t1} large teams={viewerTeams} /><b>{viewerTeams?.[fixture.t1]?.name || fixture.t1}</b><span>{innings[0]?.battingTeam === fixture.t1 ? "Batting first" : "Squad"}</span></div><div className="viewer-score"><span className={`score-state state-${state.status}`}>{statusText}</span><strong>{score ? <>{score.runs}/<WicketCount wickets={score.wickets} deliveries={current?.deliveries || []} /></> : "—"}</strong><span>{score ? `${score.overs}.${score.balls} / ${MAX_OVERS} overs` : "3V3 / 6 OV / 3 WKTS"}</span></div><div className="viewer-team"><TeamBadge code={fixture.t2} large teams={viewerTeams} /><b>{viewerTeams?.[fixture.t2]?.name || fixture.t2}</b><span>{innings[0]?.battingTeam === fixture.t2 ? "Batting first" : "Squad"}</span></div></section>
    ...
    {score ? <>
      ...
      <PlayerStats state={state} teamCodes={[fixture.t1, fixture.t2]} teams={viewerTeams} mode="viewer" />
